...
import { viajesApi, type OperacionDisponible, type CosechaLibre } from '../../../api/viajes';
...
interface ConteoCosechaWizardProps {
  viaje: {
    id: string;
    fecha: string;
...
export default function ConteoCosechaWizard({ viaje, onClose }: ConteoCosechaWizardProps) {
  const navigate = useNavigate();
  const [etapaActual, setEtapaActual] = useState(1);
  const [cosechas, setCosechas] = useState<CosechaConteo[]>([]);
  const [procesando, setProcesando] = useState(false);

  // Operaciones disponibles para asignar cosechas
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setLotesData(lotes);
        setSublotes((await Promise.all(lotes.map(async l => {
  ...
  // Cargar cosechas libres cuando cambia la operación
  useEffect(() => {
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input value={String(viaje?.placaVehiculo ?? '')} disabled />
                      </div>
                      <div className="space-y-2">
                        <Label>Conductor</Label>
                        <Input value={String(viaje?.conductor ?? '')} disabled />
                      </div>
                      <div className="space-y-2">
                        <Label>Transportador</Label>
                        <Input value={String(viaje?.transportador ?? '')} disabled />
                      </div>
                      <div className="space-y-2">
                        <Label>Extractora Destino</Label>
                        <Input value={String(viaje?.extractora ?? '')} disabled />
                      </div>
                      <div className="space-y-2">
                              ...
                              <Input
                                ...
                                onChange={(e) => {
                                  const updated = cosechas.map(c =>
                                    c.id === cosecha.id ? { ...c, fechaCosecha: e.target.value } : c
                                  );
                                  setCosechas(updated);
                                }}
                              />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-semibold text-sm">{String(viaje?.placaVehiculo ?? '')}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Conductor</span>
                        <span className="font-semibold text-sm">{String(viaje?.conductor ?? '')}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Extractora</span>
                        <span className="font-semibold text-sm truncate ml-2 max-w-[120px]" title={String(viaje?.extractora ?? '')}>
                          {String(viaje?.extractora ?? '')}
                        </span>
                      </div>
                    </div>
                  </div>
