...
import { Lock, Eye, EyeOff, CheckCircle2, AlertCircle, Store } from 'lucide-react';
import { PalmappLogo } from '../../components/common/PalmappLogo';
import { proveedorAuthApi, ProveedorAuthErrorCodes } from '../../../api/proveedorAuth';

export default function ProveedorRestablecerPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await proveedorAuthApi.resetPassword({
        token,
        email,
        password,
      ...
      setExito(true);
      setTimeout(() => navigate('/proveedor/login'), 3000);
    } catch (err: any) {
      if (err?.code === ProveedorAuthErrorCodes.PASSWORD_RESET_FAILED) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <div className="w-full max-w-md text-center space-y-4">
          ...
          <Link to="/proveedor/recuperar-password" className="text-primary hover:underline text-sm">
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        {exito ? (
          <div className="rounded-xl border border-green-500/20 bg-green-500/10 p-6 text-center space-y-3">
            ...
            <Link to="/proveedor/login" className="text-sm text-primary hover:underline">Ir al login ahora</Link>
          </div>
        ) : (
