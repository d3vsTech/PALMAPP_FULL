...
interface NominaAdministradorModalProps {
  isOpen: boolean;
  onClose: () => void;
  fechaInicio: string;
  fechaFin: string;
...
// Datos mock de nómina semanal
const nominaSemanalData = [
  { semana: 'Sem 1', total: 13000000, bonificaciones: 380000, descuentos: 120000, horasExtras: 15 },
  { semana: 'Sem 2', total: 13500000, bonificaciones: 420000, descuentos: 95000, horasExtras: 18 },
  { semana: 'Sem 3', total: 12800000, bonificaciones: 350000, descuentos: 140000, horasExtras: 12 },
  { semana: 'Sem 4', total: 14200000, bonificaciones: 450000, descuentos: 110000, horasExtras: 20 },
];
...
const historialPagosData = [
  { periodo: 'Semana 4 Mar', tipo: 'Semanal', total: 14200000, estado: 'Pendiente' },
  { periodo: 'Semana 3 Mar', tipo: 'Semanal', total: 12800000, estado: 'Pagado' },
  { periodo: 'Q1 Mar', tipo: 'Quincenal', total: 5445000, estado: 'Pagado' },
  { periodo: 'Semana 2 Mar', tipo: 'Semanal', total: 13500000, estado: 'Pagado' },
  { periodo: 'Semana 1 Mar', tipo: 'Semanal', total: 13000000, estado: 'Pagado' },
  { periodo: 'Q2 Feb', tipo: 'Quincenal', total: 5280000, estado: 'Pagado' },
  { periodo: 'Mensual Feb', tipo: 'Mensual', total: 10890000, estado: 'Pagado' },
];

export function NominaAdministradorModal({
  isOpen,
  onClose,
  fechaInicio,
  fechaFin,
}: NominaAdministradorModalProps) {
  ...
  const totalNominaSemanal = nominaSemanalData.reduce((acc, s) => acc + s.total, 0) / nominaSemanalData.length;
      ...
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl flex items-center gap-2">
            ...
            Gestión de Nómina - Administrador
          </DialogTitle>
          <DialogDescription>
            Control completo de nómina diaria, semanal, quincenal, mensual, bonificaciones, descuentos y horas extras
          </DialogDescription>
        </DialogHeader>
        ...
        {/* Resumen general - Incluye SEMANAL */}
        <div className="grid grid-cols-2 md:grid-cols-7 gap-3 mb-4">
          ...
          <div className="p-4 rounded-lg bg-gradient-to-br from-info/10 to-info/5 border border-info/20">
            ...
            <p className="text-lg font-bold text-info">${Math.round(totalNominaSemanal / 1000)}K</p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-gradient-to-br from-accent/10 to-accent/5 border border-accent/20">
            ...
            <p className="text-lg font-bold text-accent">${Math.round(totalBonificaciones / 1000)}K</p>
          </div>
          <div className="p-4 rounded-lg bg-gradient-to-br from-warning/10 to-warning/5 border border-warning/20">
            ...
            <p className="text-lg font-bold text-warning">${Math.round(totalDescuentos / 1000)}K</p>
          </div>
          <div className="p-4 rounded-lg bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20">
            <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
              <Clock className="h-3 w-3" /> Horas Extras
            </p>
            <p className="text-lg font-bold text-primary">{totalHorasExtras}h</p>
          </div>
        </div>
        ...
        <Tabs defaultValue="resumen" className="w-full">
              ...
              <div className="border rounded-lg p-4 bg-card">
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-4 rounded-lg bg-gradient-to-r from-info/20 to-info/10 border border-info/30">
                    ...
                    <p className="text-3xl font-bold text-info">${Math.round(totalNominaSemanal).toLocaleString('es-CO')}</p>
                  </div>
          ...
          <TabsContent value="semanal" className="mt-4 space-y-4">
            <div className="border rounded-lg p-4 bg-card">
              <h4 className="font-semibold mb-4">Análisis de Nómina Semanal</h4>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={nominaSemanalData}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                  ...
                  />
                  <Bar dataKey="total" fill="hsl(var(--chart-1))" name="Total Nómina" radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    ...
                    <TableHead className="text-right">Total Nómina</TableHead>
                ...
                <TableBody>
                  {nominaSemanalData.map((semana, idx) => (
                    <TableRow key={idx}>
                      <TableCell className="font-medium">{semana.semana}</TableCell>
                      <TableCell className="text-right text-xl font-bold text-success">
                        ${(semana.total / 1000000).toFixed(1)}M
                      </TableCell>
                      <TableCell className="text-right text-accent font-semibold">
                        +${(semana.bonificaciones / 1000).toFixed(0)}K
                      </TableCell>
                      <TableCell className="text-right text-warning font-semibold">
                        -${(semana.descuentos / 1000).toFixed(0)}K
                      </TableCell>
                      <TableCell className="text-right">
                        <Badge variant="secondary">{semana.horasExtras}h</Badge>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    ...
                    <TableHead className="text-right">Salario Semanal</TableHead>
                    <TableHead className="text-right">Bonificaciones</TableHead>
                    <TableHead className="text-right">Descuentos</TableHead>
                ...
                <TableBody>
                  {colaboradoresFiltrados.map((colaborador) => (
                    <TableRow key={colaborador.id}>
                      ...
                      <TableCell className="text-right font-semibold text-info">${colaborador.salarioSemanal.toLocaleString('es-CO')}</TableCell>
                      <TableCell className="text-right">
                ...
                <TableBody>
                  {historialPagosData.map((pago, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell className="text-right text-xl font-bold text-success">
                        ${(pago.total / 1000000).toFixed(1)}M
                      </TableCell>
                      <TableCell>
                        {pago.estado === 'Pagado' ? (
