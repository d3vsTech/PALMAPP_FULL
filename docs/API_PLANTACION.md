...
| `estado`    | boolean | No        | Filtra por estado (`true` / `false`)           |
| `per_page`  | integer | No        | Registros por página (default: **50**, sin tope máximo en backend) |
| `page`      | integer | No        | Número de página (default: 1)                  |

**Permiso:** `palmas.ver`

> **⚠️ IMPORTANTE — La respuesta SIEMPRE viene paginada.**
> Si no se envía `per_page`, el endpoint devuelve **solo las primeras 50 palmas** del sublote, aunque existan miles. El total real está en `meta.total`.
>
> **Para cargar todas las palmas** (ej. al abrir el modo edición de un sublote):
> 1. **Opción A (recomendada):** Hacer una primera llamada para conocer el total y luego pedir todo en una sola página:
>    ```js
>    // 1. Primera llamada (cualquier per_page sirve, usamos 1 para ahorrar payload)
>    const { meta } = await api.get('/palmas', { params: { sublote_id, per_page: 1 } });
>    // 2. Segunda llamada con per_page = total
>    const { data } = await api.get('/palmas', { params: { sublote_id, per_page: meta.total } });
>    ```
> 2. **Opción B:** Iterar todas las páginas usando `meta.last_page`.
>
> No existe un tope máximo de `per_page` en el backend, pero recuerda que un sublote puede tener hasta **99.999** palmas — pedir todo en una sola página puede impactar memoria/red. Considera virtualización en el frontend.
...
| Campo `meta`    | Tipo    | Descripción                                            |
|-----------------|---------|--------------------------------------------------------|
| `current_page`  | integer | Página actual devuelta                                 |
| `last_page`     | integer | Última página disponible (útil para iterar)            |
| `per_page`      | integer | Registros incluidos en esta página                     |
| `total`         | integer | **Total real** de palmas que coinciden con los filtros |
