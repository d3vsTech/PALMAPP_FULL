...
**Migración 7 — Nómina (6 tablas):** `nomina_concepto` (catálogo unificado de aportes legales, deducciones y bonificaciones — incluye porcentajes empleado/empresa y vigencia directamente en la fila), `nominas` (encabezado del período), `nomina_empleado` (línea por trabajador), `nomina_empleado_concepto` (detalle de cada deducción/bonificación), `nomina_jornal_ref` (snapshot de jornales incluidos), `nomina_cosecha_ref` (snapshot de cosechas incluidas). En 2026-06 se consolidó `nomina_tabla_legal` dentro de `nomina_concepto` (ver migración `2026_06_03_000007_consolidate_tabla_legal_into_nomina_concepto.php`).
...
**Migración 25 — Nómina: soporte de terceros y paso 3 "Validar Cosecha" (6 sub-migraciones, junio 2026):** Extiende el módulo de nómina para terceros y agrega la tabla de validación de cosecha. (1) Altera `nomina_empleado`: agrega `operario_id` (FK nullable a `operarios`, `restrictOnDelete`) y `tercero_id` (FK nullable a `terceros`, `restrictOnDelete`); guards `if (pgsql)` en ADD/DROP CONSTRAINT para compatibilidad SQLite. (2) Crea `nomina_tercero` (acta de pago por tercero dentro de una nómina: FK `nomina_id` + `tercero_id`, totales `total_jornales`/`total_cosecha`/`total_devengado`/`total_a_transferir`, `estado_pago` PENDIENTE|PAGADO, campos de pago `pagado_at`/`pagado_por`/`metodo_pago`/`referencia_pago`/`orden_pago_numero`). (3) Crea `nomina_tercero_operario` (línea por operario dentro del acta: `nomina_tercero_id`, `operario_id`, `dias`, `tarifa_dia`, `subtotal`, `labores_realizadas` JSONB, `ajuste`, `observacion`). (4) Crea `nomina_validacion_cosecha` (snapshot del Paso 3 del wizard — UNIQUE `nomina_id`, `total_kg_colaboradores` decimal, `total_kg_extractora` decimal, `diferencia_kg` decimal, `detalle_por_colaborador` JSONB array `[{tipo,colaborador_id,kg}]`, `validado_por` FK a `users`, `validado_at` timestamp). (5) Altera `terceros`: agrega `banco`, `tipo_cuenta`, `numero_cuenta`, `titular_cuenta` para habilitar pagos por transferencia. (6) Altera `tercero_labor_precios`: agrega `tarifa_jornal` decimal nullable (precio diario fijo para operarios del tercero cuando la labor está en `JORNAL_FIJO`; complementa `precio_palma` para `POR_PALMA`). Modelos nuevos: `NominaTercero`, `NominaTerceroOperario`, `NominaValidacionCosecha` (cast `detalle_por_colaborador` → array, accessor `isConfirmado()` = `validado_at !== null`). Modelos extendidos: `Tercero` (+`banco`/`tipo_cuenta`/`numero_cuenta`/`titular_cuenta` a `$fillable`, accessor `datos_bancarios_completos`), `TerceroLaborPrecio` (+`tarifa_jornal`, accessor `precio_resuelto`), `NominaEmpleado` (+`operario_id`/`tercero_id` a `$fillable`+casts, relaciones `operario()`/`tercero()`, helpers `esDeOperario()`/`esDeEmpleado()`). `JornalCalculationService::resolverPrecioLabor()` ramifica entre `precio_palma` y `tarifa_jornal` vía `precio_resuelto` del modelo. Tests: `NominaEmpleadoXorTest`, `NominaEmpleadoOperarioRequiereTerceroTest`, `JornalCalculationServiceTipoPagoTest`, `TarifaJornalBackfillTest`.

**Migración 26 — Promedio efectivo por nómina × lote (1 tabla nueva, junio 2026):** Crea `nomina_promedio_lote` (override manual del admin para el promedio kg/gajo de un lote, discriminado por nómina): `tenant_id`, `nomina_id` (FK cascade), `lote_id` (FK restrict), `promedio_auto` decimal(10,4) (AVG calculado de `promedio_lote` del período, o baseline del año), `promedio_manual` decimal(10,4) nullable (override del admin), `promedio_efectivo` decimal(10,4) (= manual ?? auto), `ajustado_por` FK nullable a `users`, `ajustado_at` timestamp nullable. UNIQUE `(nomina_id, lote_id)`, índice `(tenant_id, nomina_id)`. Motivación: la arquitectura anterior escribía el override en `promedio_lote` con `viaje_id=NULL`, contaminando la tabla auto-generada por `ViajeCalculationService`. Ahora `promedio_lote` es **de solo lectura** para la aplicación (solo `ViajeCalculationService` escribe en ella). Los overrides del admin van a `nomina_promedio_lote` y tienen prioridad en: `ValidarCosechaService::bundle()` (diferencia colaboradores vs extractora), `NominaCalculationService::sumarCosecha()`/`sumarCosechaOperario()` (pago efectivo en liquidación), y `CerrarNominaService::snapshotCosechas()` (campo `promedio_promedios_snapshot` en `nomina_cosecha_ref`). Modelo nuevo: `NominaPromedioLote` (BelongsToTenant, relaciones `nomina()`, `lote()`, `ajustadoPor()`; casts `promedio_auto/manual/efectivo` → `decimal:4`, `ajustado_at` → `datetime`).
...
**Precios de Abono** (`precio_abono`) es una tabla de escalas **genérica por tenant** que define el precio por palma según los gramos aplicados. Se usa exclusivamente cuando la labor FERTILIZACION está configurada en `tipo_pago=POR_PALMA`. Ejemplo: si un trabajador aplica 200g/palma, el sistema busca el rango que contiene 200g y obtiene `precio_palma`. Típicamente son pocos registros (3-6 rangos por tenant). Si FERTILIZACION pasa a `JORNAL_FIJO`, el sistema ignora esta tabla y usa `labor.precio_palma` plano.

**Labores** (`labores`) es el **catálogo único** per-tenant para palma + finca (post unificación de junio 2026 — eliminó la tabla `precios_palma` y los antiguos controllers `PrecioPalmaController`/`LaborPalmaController`). Contiene tres tipos de registros:

- **Fijas del sistema** (`es_sistema=true`, `categoria='PALMA'`, `tipo ∈ {COSECHA,PLATEO,PODA,FERTILIZACION,SANIDAD}`): se siembran al crear el tenant — 5 filas con `precio_palma=NULL` (el admin las configura desde Configuración → Labores). No se pueden borrar ni renombrar. Solo `tipo_pago`, `precio_palma` y `estado` son editables. `tipo_pago` por defecto: COSECHA/PLATEO/PODA/FERTILIZACION = `POR_PALMA`, SANIDAD = `JORNAL_FIJO`.
- **Custom de palma** (`es_sistema=false`, `categoria='PALMA'`, `tipo=NULL`, `nombre` libre): el admin las crea y gestiona desde Configuración → Labores. Aparecen junto a las fijas en el dropdown del wizard. `tipo_pago` configurable POR_PALMA / JORNAL_FIJO.
- **Custom de finca** (`es_sistema=false`, `categoria='FINCA'`, `tipo=NULL`, `nombre` libre): trabajos manuales de mantenimiento (reparaciones, transporte interno, etc.). `tipo_pago` siempre forzado a `JORNAL_FIJO` por el modelo (en `Labor::booted()`).

Reglas de cálculo (en `JornalCalculationService::calcular(Labor, array)` y `CosechaCalculationService::calcular(Labor, ...)`):

- `tipo_pago=POR_PALMA` (todas excepto FERTILIZACION y COSECHA) → `valor_total = cantidad_palmas × labor.precio_palma`.
- `tipo_pago=POR_PALMA` + `tipo=FERTILIZACION` → busca `precio_abono` por rango de gramos → `valor_total = cantidad_palmas × precio_abono.precio_palma`.
- `tipo_pago=POR_PALMA` + `tipo=COSECHA` → usa `precios_cosecha` por `(lote, año)` y peso confirmado (flujo histórico de `registro_cosecha`/`cosecha_cuadrilla`, ver §6.4). No vive en `jornales`.
- `tipo_pago=JORNAL_FIJO` (cualquier categoría / tipo / custom) → `valor_total = labor.precio_palma` plano.
- `labor.precio_palma=NULL` → el jornal/cosecha se guarda con `valor_total=NULL` y se hidrata cuando el admin configure el precio (COSECHA con JORNAL_FIJO sin precio devuelve 422 `CALC_ERROR` para no permitir cosechas sin pago resuelto).

**Precios de Cosecha** (`precios_cosecha`) define el precio pagado por kg de cosecha según el lote y año. Solo se consulta cuando la labor COSECHA está en `tipo_pago=POR_PALMA`.

Modelos: Insumo, PrecioAbono, PrecioCosecha, Labor. Controlador: `LaborController` (CRUD unificado bajo `configuracion.editar`; `GET /labores/select?categoria=PALMA|FINCA` abierto también a `operaciones.crear|editar` para los dropdowns del wizard).
...
Un **jornal** es una fila por `(operación × empleado × labor)` que describe lo que ganó un empleado ese día. La tabla `jornales` apunta SIEMPRE al catálogo unificado de `labores` vía `labor_id` (no nullable). Mantiene snapshots redundantes de `categoria` y `tipo` derivados de la labor al momento del registro:

- `labor_id` (FK → `labores`, NOT NULL): apunta a una labor fija o custom (palma o finca).
- `categoria` ∈ {`PALMA`, `FINCA`}: snapshot de `labor.categoria` al crear/actualizar.
- `tipo` ∈ {`PLATEO`, `PODA`, `FERTILIZACION`, `SANIDAD`}: snapshot de `labor.tipo`. **NULL** para labores custom (palma o finca). El enum ya NO incluye `OTROS` — las labores custom de palma quedan con `tipo=NULL`.

**COSECHA NO vive en `jornales`.** Se maneja en `registro_cosecha` (cabecera por sublote, ahora con snapshot `labor_id` a la fija COSECHA del tenant) + `cosecha_cuadrilla` (distribución por empleado). El `JornalController` rechaza con 422 `CALC_ERROR` cualquier intento de registrar COSECHA por ese endpoint.
...
**Lógica de cálculo** (centralizada en `JornalCalculationService::calcular(Labor $labor, array $data)`):

- `labor.tipo === COSECHA` → throw "usa /operaciones/{id}/cosechas".
- `labor.tipo_pago === POR_PALMA`:
  - Si `labor.tipo === FERTILIZACION`: requiere `cantidad_palmas + insumo_id + gramos_por_palma`. Busca `precio_abono` por rango de gramos → `precio_insumo_snapshot` + `valor_total = cantidad_palmas × precio_abono.precio_palma`.
  - Cualquier otra (PLATEO/PODA/SANIDAD/custom PALMA): requiere `cantidad_palmas`. `valor_total = cantidad_palmas × labor.precio_palma`.
- `labor.tipo_pago === JORNAL_FIJO`: `valor_total = labor.precio_palma` plano (cualquier categoría/tipo/custom).
- Si `labor.precio_palma IS NULL` → `valor_total = NULL` (limbo permitido; se hidrata cuando el admin configure el precio).

**Validación** (`StoreJornalRequest`): el cliente envía SOLO `labor_id` (no `categoria` ni `tipo`). El FormRequest carga la labor en `prepareForValidation`, hidrata los snapshots y aplica la matriz por `(labor.categoria, labor.tipo, labor.tipo_pago)` en `withValidator`. Esquema simplificado vs el antiguo: ya no existe el modo "OTROS catálogo" (los jornales con labor custom de palma siguen una sola ruta).

**Campos por configuración de labor:**

| Labor / `tipo_pago` | `cantidad_palmas` | `insumo_id` | `gramos_por_palma` | `descripcion` | `ubicacion` |
|---|---|---|---|---|---|
| PLATEO / PODA POR_PALMA | ✔ | ❌ | ❌ | opc | ❌ |
| PLATEO / PODA JORNAL_FIJO | ❌ | ❌ | ❌ | opc | ❌ |
| FERTILIZACION POR_PALMA | ✔ | ✔ | ✔ | opc | ❌ |
| FERTILIZACION JORNAL_FIJO | opc | opc | opc | opc | ❌ |
| SANIDAD POR_PALMA | ✔ | ❌ | ❌ | ✔ | ❌ |
| SANIDAD JORNAL_FIJO | ❌ | ❌ | ❌ | ✔ | ❌ |
| Custom PALMA POR_PALMA | ✔ | ❌ | ❌ | opc | ❌ |
| Custom PALMA JORNAL_FIJO | ❌ | ❌ | ❌ | opc | ❌ |
| Custom FINCA | ❌ | ❌ | ❌ | ❌ | opc |
...
- `RegistroCosechaController`: `POST /operaciones/{id}/cosechas`, **`POST /operaciones/{id}/cosechas/bulk`**, `PUT|DELETE /cosechas/{id}`. Resuelve la labor fija COSECHA del tenant y la pasa a `CosechaCalculationService`. Si la labor está en `POR_PALMA` (default) usa `precios_cosecha` y peso × precio como siempre. Si está en `JORNAL_FIJO`, `valor_total = labor.precio_palma` plano (peso opcional, persistido solo como tracking). Distribuye `valor_total / N` entre la cuadrilla. Usa `StoreRegistroCosechaRequest` y `UpdateRegistroCosechaRequest` (FormRequests dedicados). El bulk acepta array de hasta 200 cosechas con cuadrillas anidadas en una sola transacción.
- `JornalController`: `POST /operaciones/{id}/jornales`, **`POST /operaciones/{id}/jornales/bulk`**, `PUT|DELETE /jornales/{id}`. Carga la labor, delega a `JornalCalculationService::calcular(Labor, array)`. Devuelve 422 `CALC_ERROR` si la labor es COSECHA. El bulk acepta array mixto de jornales (cualquier tipo de labor: plateo, poda, fertilización, sanidad, custom palma, finca) en una sola transacción; responde `[{id, sync_uuid}]` en el mismo orden que el request.
...
- Selects de Lote/Sublote para el wizard: existen **dos pares de endpoints separados**, no se mezclan permisos.
  ...
  - `GET /operaciones/lotes/select` y `GET /operaciones/sublotes/select`: dedicados al Paso 2 del wizard. Solo requieren `operaciones.crear|editar`. El payload de `/sublotes/select` incluye `cantidad_palmas` para auto-rellenar el input "Número de Palmas" en PLATEO/PODA/FERTILIZACION y en SANIDAD/OTROS cuando `tipo_pago=POR_PALMA`.
- Creación de insumo desde el wizard: `POST /operaciones/insumos` (requiere `operaciones.crear|editar`). Para el flujo "Otro" del dropdown de FERTILIZACION. Duplicados → 409 `INSUMO_DUPLICADO`.
...
**Permisos:** las paramétricas (`empresa_transportadora`, `transportadores`, `extractoras`) se gestionan bajo `configuracion.editar` (no se crean permisos específicos). Los endpoints `/select` y `/{empresa}/transportadores` aceptan también `viajes.crear` porque el form "Nuevo Viaje" del módulo Viajes los consume. Los endpoints de viajes siguen usando `viajes.*` ya existentes. La columna `empresa_transportadora.tipo_persona` (`JURIDICA`/`NATURAL`, CHECK constraint, default `JURIDICA`) discrimina los badges del UI ("P. Jurídica" / "P. Natural"). El `DELETE` de las 3 paramétricas es **soft delete por `estado = false`** — las FK de `viajes` son `restrictOnDelete()` y los registros históricos no pueden quedar huérfanos. CRUD completo y payloads: [docs/API_PARAMETRICAS.md §16](docs/API_PARAMETRICAS.md). Contrato del módulo Viajes: [docs/API_VIAJES.md](docs/API_VIAJES.md).
...
**Conceptos de nómina (`nomina_concepto`):** Catálogo unificado por tenant de todo lo que puede sumar o restar en una nómina. Cada concepto tiene: **tipo** (`APORTE_LEGAL`, `DEDUCCION_LEGAL`, `DEDUCCION_VOLUNTARIA`, `BONIFICACION_FIJA`, `BONIFICACION_VARIABLE`), **subtipo** (SALUD, PENSION, ARL, FONDO_SOLIDARIDAD, LIBRANZA, EMBARGO, PRESTAMO, AHORRO_VOLUNTARIO, PRODUCTIVIDAD, TRANSPORTE, ALIMENTACION, ANTIGUEDAD, OTRO), **operación** (SUMA/RESTA), **método de cálculo** (PORCENTAJE/VALOR_FIJO/FORMULA), `valor_referencia`, `base_calculo` (SALARIO_BASE/TOTAL_DEVENGADO/SALARIO_MINIMO/MANUAL), y `aplica_a` (FIJO/VARIABLE/AMBOS). **Porcentajes y vigencia viven en la misma fila**: `porcentaje_empleado`, `porcentaje_empresa`, `vigente_desde`, `vigente_hasta`. Metadata fiscal: `afecta_salario_minimo` (pacto de no salario) y `tipo_remuneracion` (REMUNERADO/NO_REMUNERADO). El `NominaConceptoSeeder` siembra 24 conceptos por tenant (9 activos + 15 plantilla); cuando se crea un tenant vía `POST /api/admin/tenants`, el controlador siembra solo los 9 activos vía `sembrarParaTenant(soloActivos: true)`.

**Wizard frontend (4 pasos, todo el estado vive en memoria hasta el paso 4):**
1. **Elegir período** (solo UI): el usuario selecciona `{ mes, anio, periodicidad: QUINCENAL|MENSUAL, quincena? }`. No hay llamada al backend todavía; el frontend previsualiza `fecha_inicio`/`fecha_fin` calculando en cliente con los mismos `dia_inicio_q1`/`dia_fin_q1`/`dia_inicio_q2`/`dia_fin_q2` que sirve `GET /configuracion/nomina`.
2. **Elegir colaboradores** (solo UI): el frontend consulta `GET /nominas/{id}/empleados-disponibles` (o el equivalente sin id — un select global cuando aún no existe nómina) y el usuario marca empleados propios + operarios de terceros. Todo se acumula como estado local.
3. **Validar cosecha** (solo UI): el frontend consulta `GET /nominas/{id}/validar-cosecha` con una nómina temporal o pre-calcula el bundle localmente para permitir ajustes de promedio; los ajustes se guardan como estado local (`promedios_ajustados = [{lote_id, promedio}]`). No se persiste nada en `nomina_promedio_lote` todavía. Paso condicional: si no hay cosechas en el período, la UI puede saltarlo.
4. **Confirmación → "Crear Período de Pago"**: es el único paso del wizard que llama al backend. Encadena en orden cuatro llamadas dentro del handler del botón:
   - (a) `POST /nominas { mes, anio, periodicidad, quincena? }` — crea la cabecera en estado BORRADOR y devuelve `id`. El backend snapshottea `tipo_pago_snapshot` desde el body (no desde `tenant_config.tipo_pago_nomina`) y calcula `fecha_inicio`/`fecha_fin` vía `Nomina::calcularRangoFechas()` con los cortes configurables clampeados al último día real del mes.
   - (b) `POST /nominas/{id}/empleados { empleado_ids, operario_ids }` (o su alias `POST /nominas/{id}/terceros` con `{ operario_ids | tercero_ids | terceros[] }` en 3 variantes — ver [docs/API_NOMINA.md §3.4](docs/API_NOMINA.md)). Cada `nomina_empleado` arranca PENDIENTE con `salario_tipo` derivado de `empleado.modalidad_pago` (FIJO/VARIABLE) o `null` para operarios (XOR entre `empleado_id`/`operario_id`, snapshot obligatorio de `tercero_id`). El insert de operarios dispara `HidratarActaTerceroService::hidratar()`: `firstOrCreate` en `nomina_tercero` (una fila por contratista, PENDIENTE, totales 0) y en `nomina_tercero_operario` (una fila por operario, `tarifa_dia` resuelta desde `tercero_labor_precios.tarifa_jornal`), dejando las actas pre-creadas para que PR-4 solo tenga que hacer `updateOrCreate` con los totales reales.
   - (c) `PUT /nominas/{id}/promedios-lote/{lote}` × N — replayea los ajustes locales del paso 3. Se persisten en `nomina_promedio_lote` (tabla EXCLUSIVA de overrides por nómina × lote — **NO escribe** en `promedio_lote`, que es read-only). `promedio_efectivo` se resuelve con esta prioridad al leer: (1) `nomina_promedio_lote.promedio_efectivo`, (2) `AVG(promedio_lote.promedio)` en el período (auto-generado por `ViajeCalculationService`), (3) baseline del año.
   - (d) `POST /nominas/{id}/validar-cosecha/confirmar` — `ValidarCosechaService::confirmar()` hace **upsert** en `nomina_validacion_cosecha` (UNIQUE `nomina_id`) con `validado_at`. Paso condicional: solo obligatorio si hay `cosecha_cuadrilla` activas en el rango. Si hay cosechas y el paso no se ejecutó → `POST /cerrar` fallará luego con `409 NOMINA_VALIDACION_COSECHA_REQUERIDA`.

   Si (b), (c) o (d) fallan, la nómina queda a medias en BORRADOR (el backend no orquesta rollback entre llamadas). Para diagnosticar el estado de las 4 tablas después del wizard, el frontend puede consultar `GET /nominas/{id}/paso-4-checklist` — devuelve conteos por tabla + flag `listo_para_cerrar` para decidir si habilitar el botón de cerrar de la vista posterior. `DELETE /nominas/{id}/terceros/{tercero}` limpia en cascada las filas de un contratista si están PENDIENTES (409 `OPERARIO_LIQUIDADO_EN_TERCERO` si alguna ya fue liquidada); `DELETE /nomina-empleado/{id}` limpia la línea del acta si la fila es de operario y borra el `nomina_tercero` huérfano.

**Post-wizard (pantallas separadas, no del wizard):**
- **Liquidar empleado/operario individualmente** (`POST /nomina-empleado/{id}/liquidar`): el `NominaCalculationService` calcula el devengado, aplica conceptos legales automáticamente y persiste los conceptos manuales (bonificaciones libres + deducciones voluntarias del catálogo). Estado pasa a LIQUIDADO. Re-liquidación vía `PUT /liquidacion` mientras la nómina siga BORRADOR. Antes hay `GET /preview` (cálculo propuesto sin persistir) y para empleados VARIABLE `GET /resumen-trabajo` (planilla diaria agrupada por categoría). Los operarios de terceros usan una ramificación reducida (sin SALUD/PENSION/FSP/subsidio de transporte). `NominaCalculationService::sumarCosecha()` y `sumarCosechaOperario()` leen `nomina_promedio_lote` con la prioridad descrita, garantizando coherencia entre el paso 3 del wizard y el pago efectivo.
- **Liquidación de Terceros (pantalla del acta agrupada, PR-4):** flujo de 6 endpoints (`GET /nominas/{id}/terceros-actas` para el resumen; `GET/POST/PUT /nominas/{id}/terceros/{tercero}/*` para detalle, liquidar, ajustar líneas, registrar pago y descargar PDF). El `LiquidarTerceroService::liquidar()` toma como fuente de verdad los `nomina_empleado` de los operarios ya LIQUIDADOS y hace `updateOrCreate` sobre las filas que PR-3.5 pre-hidrató con totales 0, escribiendo `total_dias`/`total_jornales`/`total_cosecha`/`total_bruto`/`total_a_transferir` en `nomina_tercero` y `dias`/`tarifa_dia`/`subtotal`/`labores_realizadas` (snapshot JSON de labores distintas del período) en `nomina_tercero_operario`. Preserva `ajuste` manual y `observacion` de líneas si ya existían, y `estado_pago`/datos de giro de la cabecera. `actualizarLineaOperario()` permite editar `dias`/`tarifa_dia`/`ajuste`/`observacion` desde la UI y recalcula la cabecera. `RegistrarPagoTerceroService::registrar()` marca `estado_pago=PAGADO` con `metodo_pago`/`referencia_pago`/`orden_pago_numero`/`pagado_at`/`pagado_por` — **única excepción documentada al patrón "CERRADA = inmutable"**: sigue habilitado post-cierre. Si `metodo_pago=TRANSFERENCIA`, `RegistrarPagoTerceroRequest` valida que el tercero tenga `banco`/`tipo_cuenta`/`numero_cuenta`/`titular_cuenta` completos (422 `TERCERO_SIN_DATOS_BANCARIOS` si falta). `ActaTerceroPdfService` renderiza `resources/views/desprendible/acta_tercero.blade.php` (reusa estilos del desprendible individual) con header del contratista, tabla por operario y línea de firma. Permiso nuevo: `nomina.pagar-tercero`.
- **Cerrar nómina** (`POST /nominas/{id}/cerrar`): requiere que **todos** los empleados y operarios estén LIQUIDADOS, que la validación del wizard esté confirmada si hay cosechas, y que **cada contratista con operarios en la nómina tenga su `nomina_tercero` calculado** (`total_a_transferir > 0` o `total_dias > 0`; PENDIENTE o PAGADO — no exige pago). `CerrarNominaService` ejecuta en una sola transacción: (0) valida cosecha confirmada, (0.5) valida actas de terceros liquidadas (409 `NOMINA_TERCERO_NO_LIQUIDADO` con lista de `tercero_ids` faltantes si alguna acta no fue calculada), (1) crea snapshots en `nomina_jornal_ref`/`nomina_cosecha_ref`/`nomina_hora_extra_ref` (jornales y cosechas snapshotean por `operario_id` cuando la fila es de operario), (2) marca `Ausencia.estado=LIQUIDADA` y `HoraExtra.estado=LIQUIDADA` para los APROBADOS del rango (solo empleados propios — operarios no tienen ausencias ni horas extras), (3) recalcula totales globales, (4) cambia `nomina.estado=CERRADA` (inmutable).
...
- **Salud y Pensión:** sobre IBC = `total_devengado` (sin subsidio). Topes: mínimo 1 SMLV proporcional, máximo 25 SMLV proporcional al período. El porcentaje se lee directamente de `nomina_concepto.porcentaje_empleado` (`resolverPorcentaje()`); si la columna está NULL (concepto sin porcentaje cargado), se usa `valor_referencia` como fallback (típicamente 4% cada uno, sembrado por `NominaConceptoSeeder`).
...
- `nomina_jornal_ref(jornal_id, valor_snapshot)`, `nomina_cosecha_ref(cosecha_cuadrilla_id, valor_snapshot, promedio_promedios_snapshot)`, `nomina_hora_extra_ref(hora_extra_id, valor_snapshot)` — congelan los valores actuales. `promedio_promedios_snapshot` usa `nomina_promedio_lote.promedio_efectivo` si existe, o el AVG de `promedio_lote` en el período como fallback.
- `nomina_promedio_lote(nomina_id, lote_id, promedio_auto, promedio_manual, promedio_efectivo)` — override manual del admin para el promedio de un lote, discriminado por nómina. Tiene prioridad en todos los cálculos de pago y de cierre.
...
**Permisos Spatie:** `nomina.ver`, `nomina.crear`, `nomina.editar`, `nomina.eliminar`, `nomina.liquidar`, `nomina.cerrar`, `nomina.pagar-tercero` (registro de pago al contratista, PR-4), `nomina-conceptos.ver`, `nomina-conceptos.gestionar`.
...
Modelos: NominaConcepto, Nomina, NominaEmpleado, NominaEmpleadoConcepto, NominaJornalRef, NominaCosechaRef, NominaHoraExtraRef, NominaValidacionCosecha, NominaPromedioLote, NominaTercero, NominaTerceroOperario.

Services: `NominaCalculationService` (preview + liquidar; `sumarCosecha`/`sumarCosechaOperario` leen `NominaPromedioLote` con prioridad), `AgrupadorJornalesService` (resumen para VARIABLE), `ValidarCosechaService` (bundle con detalle por cosecha + `promedios_por_lote`, ajustar promedio en `nomina_promedio_lote`, confirmar upsert), `CerrarNominaService` (cierre transaccional + validación cosecha + validación de actas de terceros + snapshots con `nomina_promedio_lote`, snapshots por `operario_id` cuando la fila es de operario), `DesprendibleService` (data + PDF + WhatsApp placeholder), `HidratarActaTerceroService` (pre-crea `nomina_tercero` + `nomina_tercero_operario` con totales 0 y `tarifa_dia` resuelta desde `tercero_labor_precios` cuando se agrega un operario; `limpiarSiVacia()` borra el acta si queda sin operarios), `LiquidarTerceroService` (calcula el acta agregando jornales+cosecha de operarios ya liquidados; `updateOrCreate` idempotente que preserva `ajuste`/`observacion`/`estado_pago`; `actualizarLineaOperario()` para ajustes manuales que propagan a la cabecera), `RegistrarPagoTerceroService` (marca acta como PAGADO — permitido incluso con `nomina.estado=CERRADA`), `ActaTerceroPdfService` (renderiza `acta_tercero.blade.php` con DomPDF).

Controllers: `NominaController` (CRUD + indicadores con 4 cards `total_colaboradores`/`total_terceros`/`neto_pagar`/`pendiente_pagar` + `total_devengado` deprecated para compat, filtros `?anio`/`?mes`/`?estado` combinables, query agregada `SUM(CASE WHEN...)` sin N+1 — PR-5; cerrar), `NominaEmpleadoController` (agregar/eliminar empleados+operarios, alias `agregarTerceros`/`eliminarTercero` para el flujo frontend con las 3 variantes de body, `paso4Checklist` para diagnóstico del wizard, preview, resumen-trabajo, liquidar, desprendible; los bloques de inserción están extraídos a métodos privados `insertarEmpleados()` e `insertarOperarios()` reusables), `ValidarCosechaController` (bundle, ajustar promedio, confirmar — 3 endpoints Paso 3), `NominaTerceroController` (6 endpoints del acta agrupada — resumen, detalle, liquidar, ajustar línea, registrar-pago, PDF), `NominaConceptoController` (CRUD del catálogo + select; validación en `StoreNominaConceptoRequest`/`UpdateNominaConceptoRequest`).

**Sub-módulo "Configuración → Nómina" (pantalla agrupada del frontend):** 8 secciones que reutilizan endpoints ya existentes — no hay un endpoint "raíz" propio.

| Sección | Endpoint(s) | Doc |
|---|---|---|
| Periodicidad + Fechas de Corte (Q1/Q2 editables) | `GET/PUT /configuracion/nomina` (`tipo_pago_nomina`, `dia_inicio_q1`, `dia_fin_q1`, `dia_inicio_q2`, `dia_fin_q2`) | API_PARAMETRICAS §8 |
| Jornada Laboral Semanal (frontend deriva `horas = divisor / 5`) | `GET/PUT /configuracion/nomina` (`divisor_jornada_mensual`) | API_PARAMETRICAS §8 |
| **Carga inicial de la pantalla** (bundle, reemplaza 6 requests) | `GET /configuracion/precios-labores/init` | API_PARAMETRICAS §17 |
| Precios de Cosecha (lote × año × $/kg) | `GET/POST/PUT/DELETE /precios-cosecha` | API_PARAMETRICAS §9 |
| Rangos de Abonada (gramos × $/palma) | `GET/POST/PUT/DELETE /precios-abono` | API_PARAMETRICAS §3 |
| Labores (catálogo unificado: 5 fijas de palma + custom palma + custom finca, con `tipo_pago` POR_PALMA / JORNAL_FIJO por labor) | `GET/POST/PUT/DELETE /labores`, `GET /labores/select?categoria=...` | API_PARAMETRICAS §4 |
| Conceptos de Nómina (deducciones + bonificaciones) | `GET/POST/PUT/DELETE /nomina-conceptos` (permiso `nomina-conceptos.gestionar`) | API_NOMINA §7 |
| Tipos de Horas Extras (incluye `descripcion` libre) | `GET/POST/PUT/DELETE /tipos-hora-extra` | API_PARAMETRICAS §11 |
| Tipos de Novedades (motivos de ausencia, con `color`) | `GET/POST/PUT/DELETE /motivos-ausencia` | API_AUSENCIAS §1 |

Las **fechas de corte de quincena son configurables** desde `PUT /configuracion/nomina`. Defaults sembrados: `1/15/16/31`. Validación cruzada en el controller: `dia_fin_q1 ≥ dia_inicio_q1`, `dia_inicio_q2 > dia_fin_q1` (sin solapamiento), `dia_fin_q2 ≥ dia_inicio_q2`; si falla devuelve `422 CORTE_QUINCENA_INVALIDO`. Las nóminas ya creadas conservan su rango original — el cambio solo aplica a nuevas. Mapeo completo y notas de UI en [docs/API_NOMINA.md §13](docs/API_NOMINA.md).
...
**API expuesta:**
- `AusenciaController`: `POST /operaciones/{id}/ausencias`, **`POST /operaciones/{id}/ausencias/bulk`**, `PUT|DELETE /ausencias/{id}`, `POST /ausencias/{id}/aprobar`, `POST /ausencias/{id}/rechazar` (con `motivo_rechazo` obligatorio en columna dedicada), `POST /ausencias/{id}/documento` (multipart PDF/imagen, máx 5MB, guardado en `storage/app/tenants/{tenant}/ausencias/{id}/`). El bulk acepta array de hasta 200 ausencias en una sola transacción; valida que `fecha_fin` no sea anterior a `operacion.fecha`.
...
**Campo `descripcion`** (`varchar(150)` nullable en `tipos_hora_extra`): texto libre que la UI muestra debajo del nombre en el listado y en el modal de edición (ej. "Lunes a sábado 6:00 AM - 9:00 PM"). No participa en el cálculo. Sembrado por defecto en los 7 tipos por `TipoHoraExtraSeeder` y expuesto en `GET /tipos-hora-extra/select`.
...
**API expuesta:**
- `HoraExtraController`: `POST /operaciones/{id}/horas-extra`, **`POST /operaciones/{id}/horas-extra/bulk`**, `PUT|DELETE /horas-extra/{id}`, `POST /horas-extra/{id}/aprobar|rechazar`. El bulk acepta array de hasta 200 horas extras en una sola transacción; pre-valida todos los `tipo_hora_extra_id` contra el tenant en una sola query batch antes de abrir la transacción. Los snapshots (`codigo`, `porcentaje_recargo`, `valor_calculado`) los gestiona automáticamente el hook `HoraExtra::booted()`.
...
│   │   ├── NominaConcepto.php              ← Catálogo aportes/deducciones/bonificaciones (porcentajes + vigencia in-row)
...
GET             /api/v1/tenant/operaciones/wizard-init     → Bundle del wizard en modo creación (parametricas: colaboradores+lotes+sublotes+insumos+labores_palma+labores_finca+motivos_ausencia+tipos_hora_extra; cacheado 15 min/tenant). Planilla y resumen vienen null. (operaciones.ver)
GET             /api/v1/tenant/operaciones/:id/wizard-init → Bundle del wizard en modo edición/lectura: parametricas + planilla con relaciones eager-loaded + resumen calculado. Reemplaza las ~10 peticiones de apertura (7 selects + sublotes + show + resumen) por una sola. (operaciones.ver)
...
GET             /api/v1/tenant/configuracion/precios-labores/init → Bundle inicial pantalla "Precios de Labores" — devuelve precios_cosecha, precios_abono, labores_palma_fijas, labores_palma_custom, labores_finca y lotes en 1 request (caché 60 s por tenant, invalida en mutaciones de Labor/PrecioCosecha/PrecioAbono/Lote) (configuracion.editar)
...
GET             /api/v1/tenant/labores/select      → Dropdown unificado de labores (?categoria=PALMA|FINCA) (configuracion.editar O operaciones.{crear|editar})
GET|POST        /api/v1/tenant/labores             → Listar (paginado, filtros categoria/tipo/tipo_pago/es_sistema/estado) / Crear labor custom (configuracion.editar)
GET|PUT|DELETE  /api/v1/tenant/labores/:id         → Ver / Editar (fijas: solo tipo_pago+precio_palma+estado) / Eliminar custom (configuracion.editar)
...
GET             /api/v1/tenant/nominas/:id/validar-cosecha                      (nomina.editar) — bundle kg colaboradores vs extractora + estado confirmación
PUT             /api/v1/tenant/nominas/:id/promedios-lote/:lote                 (nomina.editar) — ajustar promedio baseline del lote (viaje_id=null)
POST            /api/v1/tenant/nominas/:id/validar-cosecha/confirmar            (nomina.editar) — upsert en nomina_validacion_cosecha
...
- **Paramétricas del Colaborador (EPS, Fondos de Pensión, Fondos de Cesantías, ARL, Entidades Bancarias)** — 5 catálogos paramétricos por tenant con base común (`id`, `tenant_id`, `nombre`, `estado`, timestamps + `unique(['tenant_id','nombre'])`). **EPS, Fondos de Pensión, Fondos de Cesantías y ARL** solo guardan `nombre` + `estado`; **Entidades Bancarias** además tiene `codigo` (string(10), nullable) y `contacto` (string(50), nullable) — alimentados desde la pantalla "Configuración › Colaboradores › Entidades Bancarias" donde el modal pide Nombre*, Código (opcional) y Contacto (opcional). Cada uno expone CRUD completo bajo `configuracion.editar` con auditoría (`AuditoriaService`) y un endpoint `/select` accesible además con `colaboradores.{ver|crear|editar}` para alimentar los dropdowns del formulario de colaboradores; el `/select` de Entidades Bancarias proyecta `id, nombre, codigo, contacto` (los demás solo `id, nombre`). **El empleado guarda el `nombre`** (string), no el `id` — preserva histórico al renombrar/eliminar entradas del catálogo. Listas iniciales colombianas en constantes `INICIALES` de cada modelo (17 EPS, 5 fondos de pensión, 5 fondos de cesantías, 9 ARLs, 23 bancos). Provisionamiento automático al crear tenant vía `Admin\TenantController::seedParametricasColaborador()` y seeder global idempotente `ParametricasColaboradorSeeder` para fincas existentes. Modelos: `Eps`, `FondoPension`, `FondoCesantias`, `Arl`, `EntidadBancaria`. Controllers: `EpsController`, `FondoPensionController`, `FondoCesantiasController`, `ArlController`, `EntidadBancariaController`
...
- **PR-1 — Fundaciones Nómina Terceros** — 6 sub-migraciones (2026-06-25) que extienden el módulo de nómina para terceros (ver Migración 25 en §5.1): `nomina_empleado` +`operario_id`/`tercero_id`, nuevas tablas `nomina_tercero`/`nomina_tercero_operario`/`nomina_validacion_cosecha`, extensión de `terceros` con datos bancarios, `tercero_labor_precios` +`tarifa_jornal`. 3 nuevos modelos (`NominaTercero`, `NominaTerceroOperario`, `NominaValidacionCosecha`). Fix `JornalCalculationService` para usar `tarifa_jornal` en labores `JORNAL_FIJO`. 4 tests de unidad (`NominaEmpleadoXorTest`, `NominaEmpleadoOperarioRequiereTerceroTest`, `JornalCalculationServiceTipoPagoTest`, `TarifaJornalBackfillTest`).

- **Bulk endpoints del wizard de Operaciones (julio 2026)** — 4 nuevas rutas `POST .../bulk` (jornales, cosechas, ausencias, horas-extra) que insertan arrays de hasta 200 ítems en una sola `DB::transaction()` por tipo. Reducen el tiempo de guardado del wizard de >15 s (N peticiones secuenciales) a <500 ms (4 llamadas paralelas). Permiso: `operaciones.crear`. Rollback total si cualquier ítem falla. Rutas registradas antes de las individuales en `routes/api.php` para evitar conflictos. El frontend (`guardarTodo()` en `NuevaPlanillaWizard.tsx`) los consume con `Promise.all()` y mapea los IDs de respuesta por posición. Documentación completa: [docs/API_OPERACIONES.md §12](docs/API_OPERACIONES.md).

- **PR-2 — Paso 3 "Validar Cosecha" + Promedio Efectivo por Nómina (junio 2026)** — Migración `nomina_promedio_lote` + modelo `NominaPromedioLote`. `ValidarCosechaService` completamente reescrito: `bundle()` devuelve `promedios_por_lote[]` + `detalle_por_colaborador[].cosechas[]` (fila por cosecha por colaborador con fecha, lote, sublote, remisión del viaje, gajos trabajados/verificados, kg_trabajado con `promedio_efectivo`, kg_extractora); `ajustarPromedio()` escribe en `nomina_promedio_lote` (NO en `promedio_lote`, que es ahora read-only); `buildPromedioEfectivoMap()` con cadena de prioridad `nomina_promedio_lote` → `AVG(promedio_lote)` → baseline año; `confirmar()` upsert en `nomina_validacion_cosecha`. `ValidarCosechaController::ajustarPromedio()` responde con shape de `NominaPromedioLote` (no con `PromedioLote`). `NominaCalculationService::sumarCosecha()` y `sumarCosechaOperario()` aceptan `?int $nominaId` y consultan `NominaPromedioLote` primero; `previewLiquidacion()` y `previewLiquidacionOperario()` pasan `$ne->nomina_id`. `CerrarNominaService::snapshotCosechas()` usa `NominaPromedioLote` antes del AVG fallback. `docs/API_NOMINA.md §4` actualizado con nuevo formato de bundle, `promedios_por_lote`, `cosechas[]` por colaborador, nueva respuesta de `PUT /promedios-lote/{lote}`, notas de solo lectura de `promedio_lote`.
...
1. **Caché aplicativa (servidor)** — `Cache::remember` envolviendo los reads de paramétricas. La invalidación se dispara explícitamente desde los controllers en `store/update/destroy`. TTLs:
   ...
   - Bundle "Precios de Labores" (`configuracion/precios-labores/init`): **60 s** — agrega 6 datasets (precios_cosecha, precios_abono, labores_palma_fijas, labores_palma_custom, labores_finca, lotes); invalidación en mutaciones de Labor, PrecioCosecha, PrecioAbono, Lote.
...
| `GET /fondos-cesantias/select` | 15 min | `store/update/destroy` de `FondoCesantiasController` |
...
| `GET /operaciones/{id}/wizard-init` y `/operaciones/wizard-init` | 15 min por cada catálogo (`WizardCache::operacionesColaboradores/Lotes/Sublotes/Insumos/Labores/MotivosAusencia/TiposHoraExtra`) — la planilla en sí no se cachea, solo las paramétricas | `WizardCache::forgetOperacionesKey()` en `store/update/destroy` de `EmpleadoController`, `LoteController`, `SubloteController`, `InsumoController`, `LaborController`, `MotivoAusenciaController`, `TipoHoraExtraController` |
...
| `GET /configuracion/precios-labores/init` | 60 s | `WizardCache::forgetPreciosLaboresBundle()` en `store/update/destroy` de `LaborController`, `PrecioCosechaController`, `PrecioAbonoController`, `LoteController` |
...
**Wizard de Operaciones — guardado lento (>5 s con 20+ ítems):**
1. Verificar que el frontend está enviando las 4 llamadas bulk (`POST .../jornales/bulk`, `.../cosechas/bulk`, `.../horas-extra/bulk`, `.../ausencias/bulk`) en lugar de N peticiones individuales secuenciales.
2. DevTools → Network → confirmar que las 4 POSTs se lanzan en paralelo (columna "Waterfall" sin cascada).
3. Si las rutas bulk responden 404, verificar que en `routes/api.php` están registradas **antes** de las rutas individuales (`/jornales` antes de la ruta con `{jornal}`).
4. Si algún bulk responde 422 con `CALC_ERROR`, revisar el ítem indicado en el mensaje (`"items.N: ..."`).

Ver §12 de [docs/API_OPERACIONES.md](docs/API_OPERACIONES.md) para el contrato completo.
