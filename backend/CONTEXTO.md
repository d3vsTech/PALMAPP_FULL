...
1. **Crear período (`POST /nominas`)**: el frontend envía `{ mes, anio, periodicidad: QUINCENAL|MENSUAL, quincena? }`. El backend snapshottea `tipo_pago_snapshot` desde el body (no desde `tenant_config.tipo_pago_nomina` — el config solo es default informativo del frontend) y calcula `fecha_inicio`/`fecha_fin` automáticamente vía `Nomina::calcularRangoFechas()`, que para QUINCENAL lee los días configurables (`dia_inicio_q1`, `dia_fin_q1`, `dia_inicio_q2`, `dia_fin_q2`) de `tenant_config` y clampea al último día real del mes. Estado: BORRADOR.
...
Modelos: NominaConcepto, Nomina, NominaEmpleado, NominaEmpleadoConcepto, NominaJornalRef, NominaCosechaRef, NominaHoraExtraRef.
...
Controllers: `NominaController` (CRUD + indicadores + cerrar), `NominaEmpleadoController` (agregar/eliminar empleados, preview, resumen-trabajo, liquidar, desprendible), `NominaConceptoController` (CRUD del catálogo + select; validación en `StoreNominaConceptoRequest`/`UpdateNominaConceptoRequest`).
...
| Conceptos de Nómina (deducciones + bonificaciones) | `GET/POST/PUT/DELETE /nomina-conceptos` (permiso `nomina-conceptos.gestionar`) | API_NOMINA §6 |
