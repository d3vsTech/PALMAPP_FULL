...
| `password` | string | Si | Min 8, debe incluir `password_confirmation` |
| `tenant_id` | integer | No | Si se envia, asigna al tenant |
| `rol` | string | No | `ADMIN`, `LIDER DE CAMPO`, `PROPIETARIO` (default: PROPIETARIO) |

**Respuesta 201:** Misma estructura que login.
...
```json
{
  ...
  "municipio": "Yopal",
  "fecha_activacion": "01/01/2026",
  "fecha_suspension": "01/02/2026"
}
```
...
| `tipo_persona` | **Si** | `NATURAL` o `JURIDICA` |
| `nit` | No | Unico, max 20 |
...
```json
{
  ...
  "nit": "900123456-1",
  "razon_social": "Agricola Esperanza S.A.S",
  "correo_contacto": "nuevo@esperanza.com",
  "telefono": "3101234567",
  "direccion": "Vereda Nueva, Km 5",
  "departamento": "Casanare",
  "municipio": "Aguazul",
  "fecha_activacion": "15/03/2026",
  "fecha_suspension": "15/06/2026"
}
```

| Campo | Requerido | Notas |
|-------|-----------|-------|
| `nombre` | No | Max 100 chars |
| `tipo_persona` | No | `NATURAL` o `JURIDICA` |
| `nit` | No | Unico, max 20 |
| `razon_social` | No | Max 200 chars |
| `correo_contacto` | No | Email valido, max 100 |
| `telefono` | No | Max 20 chars |
| `direccion` | No | Max 200 chars |
| `departamento` | No | Max 100 chars |
| `municipio` | No | Max 100 chars |
| `fecha_activacion` | No | Formato `dd/mm/yyyy` |
| `fecha_suspension` | No | Formato `dd/mm/yyyy` |

> Todos los campos son opcionales (`sometimes`). Solo se actualizan los campos enviados.
