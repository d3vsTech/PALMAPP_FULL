...
class TerceroConfiguracionController extends Controller
{
    ...
    // ──────────────────────────────────────────────────────────────────
    // Wizard Complete — bulk insert al finalizar el wizard de creación
    // ──────────────────────────────────────────────────────────────────

    /**
     * POST /terceros/{tercero}/wizard-complete
     * Recibe todos los precios y operarios del wizard en un solo body y los
     * persiste en una transacción. Reduce ~10 peticiones individuales a 1.
     */
    public function wizardComplete(Request $request, Tercero $tercero): JsonResponse
    {
        try {
            ...
            $data = $request->validate([
                ...
                'precios_cosecha.*.anio'       => 'nullable|integer|min:2000|max:2100',
            ...
            $conteos = DB::transaction(function () use ($data, $tenantId, $tercero) {
                ...
                foreach ($data['labor_precios'] ?? [] as $item) {
                    if (!empty($item['tipo_pago']) && $item['tipo_pago'] === Labor::TIPO_PAGO_POR_PALMA) {
                        ...
                        if ($labor && $labor->esFinca()) {
                            throw new \RuntimeException('FINCA_TIPO_PAGO_INVALIDO');
                        }
                    }
                ...
                // ── Precios abono — solapamiento contra BD y contra el batch ──
            ...
            // Una sola invalidación de caché tras el commit
            WizardCache::forgetTerceroConfigBundle($tenantId, $tercero->id);
            WizardCache::forgetOperacionesBundle($tenantId);
            ...
            if ($e->getMessage() === 'FINCA_TIPO_PAGO_INVALIDO') {
                return response()->json([
                    'message' => 'Una labor de categoría FINCA no puede tener tipo_pago POR_PALMA.',
                    'code'    => 'FINCA_TIPO_PAGO_INVALIDO',
                ], 422);
            }
